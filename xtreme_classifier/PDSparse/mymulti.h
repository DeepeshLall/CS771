...
//1:0.4 2:0.5 100:0.5  1, 2, 3
//3:0.3 2:0.3 6:0.5  4, 5, 6
...
void readData(vector<vector<int> > X1, vector<vector<float> > X2, Problem* prob) {
	int d = -1;
	for(int i = 0; i < X1.size(); i++) {
		SparseVec* ins = new SparseVec();
		//SparseVec is basically a vector<pair<int, float>>
		for(int j = 0; j < X1[i].size(); j++) {
			int ind = X1[i][j];
			ins->push_back(make_pair(X1[i][j], X2[i][j]));
			if( ind > d ) d = ind;
		}
...
void readData(char* fname, Problem* prob)
{
	...
	while( !fin.eof() ){
		...
		//let's say there is only one user (1 datapoint) in the testfile and she likes items "1", "3", "100"
		//that means the only items present in the testdata are "1", "3", "100"
		//then that means that items 2, 4, 5, ... , 99 are not liked by anyone hence we can forget them
		//In this case, lab_indices will be [0, 1, 2] (lab_indices is per user in the testdata)
		//and label_index_map will be "1" -> 0, "3" -> 1, "100" -> 2 (label_index_map is for the whole file)
	...
	//K is basically the number of reduced items (items not liked by any user have been discarded)
	...
	//label_name_list is basically the reverse mapping of label_index_map
	//i.e. if label_index_map["341"] = 5, then label_name_list[5] = "341"
	delete[] line;
}
*/
