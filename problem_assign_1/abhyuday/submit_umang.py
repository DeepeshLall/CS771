...
def getStepLength( jt ):
	global adaGrad
	return eta/math.sqrt(1 + adaGrad[jt])
...
################################
def solver( X1, y1, C1, timeout, spacing, eta_set ):
    global X,y,C,n,d,xw, gradW, gradB, eta, adaGrad
    X = X1
    y = y1
    C = C1
    eta = eta_set
...
################################
        ...
        if(jt < d):
            ...
            adaGrad[jt] = (1 - rho)* adaGrad[jt] + rho * (gradW*gradW)
            old = w_run[jt]
            w_run[jt] = w_run[jt] - gradW * getStepLength(jt)
            deltaXw = (w_run[jt] - old) * X[:,jt]
            xw = xw + deltaXw
        else:
            gradB = getCDGrad(w_run,b_run,jt)
            adaGrad[jt] = (1 - rho)* adaGrad[jt] + rho * (gradB*gradB)
            b_run_old = b_run
            b_run = b_run - gradB * getStepLength(jt)
