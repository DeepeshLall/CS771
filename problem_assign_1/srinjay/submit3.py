...
def getObj( X, y,C, w, b ):
	hingeLoss = np.maximum( (1 - np.multiply( (X.dot( w ) + b), y ))**2, 0 )
...
def doGD( X,y,C,gradFunc, stepFunc, init, horizon = 100 ):
...
################################
		...
		#theta = 
		#horizon = 1
		#for iterator in range( horizon ):
			#tic = tm.perf_counter()
		delta = getCSVMSGrad( X ,y,C, theta )
		theta = theta - getStepLength( delta, t ) * delta
		cumulative = cumulative + theta
			#toc = tm.perf_counter()
			#totTime = totTime + (toc - tic)
			#objValSeries[t] = getCSVMObjVal( X,y,C,cumulative/(t+2) )
